...
import { Search, X, Calendar, Pill, CreditCard, Ban, RefreshCw, User } from 'lucide-react'
import { useAuth } from '@/lib/auth-context'
import { getRoleHome, naira, type PatientProfile, type StaffProfile } from '@/lib/auth'
import { toast } from 'sonner'
import { getAppointments, getPrescriptions, getInvoices } from '@/lib/store'
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    'Active': 'badge-success', 'Inactive': 'badge-muted', 'Confirmed': 'badge-info',
    'Completed': 'badge-muted', 'Paid': 'badge-success', 'Pending': 'badge-warning',
    'Dispensed': 'badge-success', 'Ready': 'badge-warning', 'Cancelled': 'badge-danger',
  }
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    if (selectedPatient) {
      setPatientAppointments(getAppointments(selectedPatient.patientId))
      setPatientPrescriptions(getPrescriptions(selectedPatient.patientId))
      setPatientInvoices(getInvoices(selectedPatient.patientId))
    }
  }, [selectedPatient])

  const users = useMemo<UserRow[]>(() => {
    const staffRows = staff.map((member) => ({
      id: member.staffId, name: member.name, email: member.email,
      role: member.isHeadAdmin ? 'HEAD ADMIN' : member.role,
      department: member.department || 'General', phone: member.phone || 'Not provided',
      registeredAt: member.createdAt, status: member.isActive ? 'Active' as const : 'Inactive' as const,
      isHeadAdmin: member.isHeadAdmin, userType: 'staff' as const,
    }))
    const patientRows = patients.map((p) => ({
      id: p.patientId, name: p.name, email: p.email, role: 'PATIENT',
      department: 'Patient Portal', phone: p.phone, registeredAt: p.createdAt,
      status: 'Active' as const, userType: 'patient' as const,
    }))
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative max-w-md">
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search..." className="input-field w-full pl-8" />
      </div>
            ...
            <tbody className="divide-y divide-[--border]">
              {filteredUsers.length === 0 ? (
                ...
                <tr key={user.id} className="bg-[--surface] hover:bg-[--surface-2] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`badge ${user.role === 'PATIENT' ? 'badge-info' : 'badge-success'}`}>{user.role}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-[13px] text-[--text-3]">{new Date(user.registeredAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3"><StatusBadge status={user.status} /></td>
                  <td className="px-4 py-3">
                    {user.userType === 'patient' ? (
                      <button onClick={() => { const p = patients.find((p) => p.patientId === user.id); if (p) setSelectedPatient(p) }}
                        className="btn-ghost text-[12px] py-1.5">View Profile</button>
      ...
      {/* Patient Detail Drawer - REAL DATA */}
      {selectedPatient && (
        <>
            ...
            <div className="p-5 space-y-5">
              <div className="card">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ['Email', selectedPatient.email], ['Phone', selectedPatient.phone],
                    ['Blood Type', selectedPatient.bloodType || '—'], ['Gender', selectedPatient.gender],
                    ['DOB', selectedPatient.dateOfBirth || '—'], ['Address', selectedPatient.address || '—'],
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="section-title">Appointments ({patientAppointments.length})</h3>
                  <Calendar className="w-4 h-4 text-[--text-3]" />
                </div>
                {patientAppointments.length === 0 ? (
                  <p className="text-[13px] text-[--text-3] py-2">No appointments</p>
                ) : (
                  <div className="overflow-x-auto">
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="section-title">Prescriptions ({patientPrescriptions.length})</h3>
                  <Pill className="w-4 h-4 text-[--text-3]" />
                </div>
                {patientPrescriptions.length === 0 ? (
                  <p className="text-[13px] text-[--text-3] py-2">No prescriptions</p>
                  ...
                  <div key={rx.id} className="flex items-center gap-3 py-2.5 border-b border-[--border] last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-medium text-[--text-1]">{rx.diagnosis}</p>
                      <p className="caption">{rx.doctor} · {rx.drugs?.length || 0} drugs</p>
                    </div>
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="section-title">Invoices ({patientInvoices.length})</h3>
                  <CreditCard className="w-4 h-4 text-[--text-3]" />
                </div>
                {patientInvoices.length === 0 ? (
                  <p className="text-[13px] text-[--text-3] py-2">No invoices</p>
              ...
              <div className="flex gap-3">
                <button onClick={() => toast.success('Account disabled')} className="btn-danger flex items-center gap-1.5 flex-1 justify-center">
                  <Ban className="w-3.5 h-3.5" /> Disable Account
                </button>
                <button onClick={() => { toast.success('OTP reset sent to patient email'); setSelectedPatient(null) }}
                  className="btn-ghost flex items-center gap-1.5 flex-1 justify-center">
                  <RefreshCw className="w-3.5 h-3.5" /> Reset OTP
                </button>
              </div>
            </div>
          </div>
